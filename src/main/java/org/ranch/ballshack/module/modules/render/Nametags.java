...
public class Nametags extends Module {
	...
	public void onHudRender(EventHudRender.Pre event) {
		...
		for (Entity e : entities.toList()) {
			if (e != mc.player) {
				...
				Vec3d pos = e.getLerpedPos(event.tickCounter.getTickProgress(true)).add(0, e.getHeight() + 0.2, 0);
	...
	private void drawNametag(DrawContext context, Entity e, Matrix3x2fStack stack, float scale) {
		...
		DrawUtil.drawOutlineWithCorners(context, -1, -1, mc.textRenderer.getWidth(tagT) + 3, mc.textRenderer.fontHeight + 2, Color.DARK_GRAY, Color.DARK_GRAY);
